const { ActionRowBuilder, ButtonBuilder, ButtonStyle, ModalBuilder, TextInputBuilder, TextInputStyle, EmbedBuilder, ChannelType, StringSelectMenuBuilder } = require('discord.js');
...
async function handleTicketInteraction(interaction) {
    ...
    } else if (interaction.isStringSelectMenu() && interaction.customId.startsWith('payment_method_')) {
...
async function handleTicketConfirm(interaction, orderId) {
    ...
    if (serviceType === 'powerleveling' || serviceType === 'paragon_leveling') {
        ...
        const fourthActionRow = new ActionRowBuilder().addComponents(fromLevelInput);
        const fifthActionRow = new ActionRowBuilder().addComponents(toLevelInput);
        ...
        const fourthActionRow = new ActionRowBuilder().addComponents(killsInput);
...
async function handleOrderForm(interaction, orderId) {
    try {
        ...
        const expressType = interaction.fields.getTextInputValue('express_type');
        ...
        if (serviceType === 'powerleveling' || serviceType === 'paragon_leveling') {
            const fromLevel = interaction.fields.getTextInputValue('from_level');
            const toLevel = interaction.fields.getTextInputValue('to_level');
            updateData.from_level = fromLevel;
            updateData.to_level = toLevel;
        }
        else if (serviceType === 'boss_kills') {
            const killsAmount = interaction.fields.getTextInputValue('kills_amount');
            updateData.kills_amount = killsAmount;
        }
        else if (serviceType === 'boss_mats') {
            const matsAmount = interaction.fields.getTextInputValue('mats_amount');
            updateData.mats_amount = matsAmount;
        }
        else if (serviceType === 'custom_order') {
            const customDescription = interaction.fields.getTextInputValue('custom_description');
        ...
        const staffRole = interaction.guild.roles.cache.find(role => role.name === config.roleNames.staff);
        
        // Try multiple ways to find the orders-details channel
        ...
        if (!ordersDetailsChannel) {
            ordersDetailsChannel = interaction.guild.channels.cache.find(
                channel => channel.name === config.channelNames.ordersDetailsAlt
            );
        }
        ...
        if (!ordersDetailsChannel) {
            ordersDetailsChannel = interaction.guild.channels.cache.find(
                channel => channel.name.toLowerCase().includes('order') && channel.name.toLowerCase().includes('detail')
            );
        }
        ...
        console.log('Orders details channel found:', ordersDetailsChannel ? ordersDetailsChannel.name : 'Not found');
        console.log('Looking for channel name:', config.channelNames.ordersDetails);

        // Send single notification to orders-details channel
        if (ordersDetailsChannel) {
            try {
                ...
                console.log('Successfully sent to orders-details channel');
            } catch (error) {
                console.error('Error sending to orders-details channel:', error);
            }
        } else {
            console.error('orders-details channel not found!');
        }

        // Send direct messages to all staff members
        if (staffRole) {
            try {
                const serviceConfig = config.services.find(s => s.value === updatedOrder.service_type);
                ...
                const staffDMEmbed = createOrderDetailsEmbed(updatedOrder, user)
                    ...
                    .setTitle(`🚨 NEW ORDER - ${serviceConfig ? serviceConfig.label : updatedOrder.service_type}`)
                    .addFields(
                        { name: '🔔 Staff Notification', value: 'New order requires your attention', inline: false }
                    );

                staffMembers.forEach(async (member) => {
                    try {
                        await member.send({
                            content: 'New order notification from PotatoBoosting',
                            embeds: [staffDMEmbed]
                        });
                        console.log(`Sent DM to staff member: ${member.user.tag}`);
                    } catch (dmError) {
                        console.log(`Could not send DM to ${member.user.tag}: ${dmError.message}`);
                    }
                });
                
                console.log(`Attempted to send DM notifications to ${staffMembers.size} staff members`);
            } catch (error) {
        ...
        const paymentSelectMenu = new ActionRowBuilder()
            .addComponents(
                new StringSelectMenuBuilder()
                    .setCustomId(`payment_method_${orderId}`)
                    .setPlaceholder('Select your payment method...')
                    .addOptions(config.paymentMethods.map(method => ({
                        label: method.label,
                        value: method.value,
                        emoji: method.emoji
                    })))
        ...
        await interaction.reply({
            content: '✅ Order confirmed! Please select your payment method below:',
            embeds: [embed],
...
async function handleStaffComplete(interaction, orderId) {
    ...
    try {
        ...
        let completedCategory = interaction.guild.channels.cache.find(
            c => c.type === ChannelType.GuildCategory && c.name === config.categories.completedOrders
        );
        ...
        setTimeout(async () => {
            try {
                ...
                if (reviewsChannel) {
                    const reviewButton = new ActionRowBuilder()
                        .addComponents(
                            new ButtonBuilder()
                                .setLabel('Leave a Review')
                                .setStyle(ButtonStyle.Link)
                                .setURL(`https://discord.com/channels/${interaction.guild.id}/${reviewsChannel.id}`)
                                .setEmoji('⭐')
                        );
                    ...
                    await user.send({
                        ...
                        console.log(`Could not send review message to user ${user.tag}:`, error.message);
                    });
                }
...
async function handlePaymentMethodSelection(interaction, orderId) {
    try {
        ...
        const paymentConfig = config.paymentMethods.find(p => p.value === paymentMethod);
        ...
        await interaction.reply({
            content: '❌ Failed to process payment method selection. Please try again.',
            ephemeral: true
        });
    }
}
